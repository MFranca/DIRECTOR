 ...
 */
@WebServlet("/listCandidates")
public class CandidateController extends DirectorAbstractController {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {			
			String positiveFilter = request.getParameter("txtPositiveFilter");
            String negativeFilter = request.getParameter("txtNegativeFilter");			
			
			ServiceResource resource = new ServiceResource();
			
			htmlResponse += "<spam style=\"font-family: Verdana, Geneva, Tahoma, sans-serif;font-size: 20px;\">";
			htmlResponse += resource.getServiceCandidates("webApplication", positiveFilter, negativeFilter);
			htmlResponse += "</spam>";
